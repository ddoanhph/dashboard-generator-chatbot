...
#         user_message = data.get('message', '')
#         conversation_history = data.get('history', [])
#         current_dashboard = data.get('current_dashboard', None)  # Add this!
        
#         # Build context with existing dashboard
#         context = f"{SYSTEM_PROMPT}\n\n"
        
#         # If there's an existing dashboard, tell the AI about it
#         if current_dashboard:
#             context += f"""
# CURRENT DASHBOARD STATE:
# {json.dumps(current_dashboard, indent=2)}

# IMPORTANT: The user wants to MODIFY this existing dashboard, not create a new one.
# - If they say "add a chart", ADD to the existing visualizations array
# - If they say "change the pie chart", MODIFY that specific visualization
# - If they say "add a metric", ADD to the existing metrics array
# - Keep all existing elements unless explicitly asked to remove them
# - Return the COMPLETE updated dashboard with both old and new elements
...
#                 "dashboard": current_dashboard  # Keep existing dashboard if parsing fails
#             }
